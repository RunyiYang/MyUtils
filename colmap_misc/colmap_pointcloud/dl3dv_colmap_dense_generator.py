...
def arg_parser():
    ...
    parser.add_argument('--input_dir', type=str, default='/srv/beegfs-benderdata/scratch/qimaqi_data/data/iccv_2025/datasets/Dataset/scripts/DL3DV-10K/1K/', help='Input directory')
...
def run_colmap():
    ...
    for seq_i in tqdm(sequences):
        seq_dir = os.path.join(input_dir, seq_i)
        print("=====================================")
        ...
        os.system(f"slurm-podman-run -it --rm colmap-cuda-interactive colmap feature_extractor --database_path {colmap_database_path} --image_path {colmap_image_path}")

        # check if the database is created
        assert os.path.exists(colmap_database_path), f"Database not created: {colmap_database_path}"
        
        os.system(f"colmap exhaustive_matcher --database_path {colmap_database_path} --SiftMatching.gpu_index={gpu_idx}")

        triangulated_path = os.path.join(seq_dir, 'colmap', 'triangulated', 'sparse', 'model')
        os.makedirs(triangulated_path, exist_ok=True)
        os.system(f"colmap point_triangulator --database_path {colmap_database_path} --image_path {colmap_image_path} --output_path {triangulated_path} --input_path {os.path.join(seq_dir, 'colmap', 'sparse', 'known')}")
        ...
        os.system(f"colmap image_undistorter --input_path {triangulated_path} --output_path {dense_output_path} --image_path {colmap_image_path}")

        os.system(f"colmap patch_match_stereo --workspace_path {dense_output_path} --PatchMatchStereo.gpu_index={gpu_idx}")

        os.system(f"colmap stereo_fusion --workspace_path {dense_output_path} --output_path {fuse_ply_path}")
